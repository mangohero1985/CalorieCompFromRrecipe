...
public class InfoSeek {
	...
	public void Remove(BufferedReader br, BufferedWriter bw, String AdditionWord) throws IOException {

		String readsString;
		String ReturnResultOfKeyWord;
		String ReturnRerultOfKeyWordAndAdditon;
		
		while((readsString = br.readLine())!= null){
			if(readsString.matches("[a-zA-Z]+$"))
				continue;
			String KeyWord = "\""+readsString+"\"";
			String KeyWordAndAddition = "\""+readsString+"\"+"+AdditionWord;
			//搜索keyword的Number
			ReturnResultOfKeyWord=GoogleResultReturn(KeyWord);
			...
			int NumOfKeyword = Integer.parseInt(splitePureNum1[0]);
			//搜索keyword和料理的number
			ReturnRerultOfKeyWordAndAdditon = GoogleResultReturn(KeyWordAndAddition);
			if(!ReturnRerultOfKeyWordAndAdditon.contains("約&nbsp;"))
			   continue;
			String[] spliteReturnResult2 = ReturnRerultOfKeyWordAndAdditon.split("約&nbsp;");
			String[] splitePureNum2 = spliteReturnResult2[1].substring(0,10).split("&");
			int NumOfKeyWordAndAdditon = Integer.parseInt(splitePureNum2[0]);
			
			double result =(double)NumOfKeyWordAndAdditon/(double)NumOfKeyword;
			BigDecimal bigDecimal = new BigDecimal(result);
			
			System.out.println(readsString+" "+bigDecimal.setScale(7, BigDecimal.ROUND_HALF_UP));
			bw.write(readsString+" "+bigDecimal.setScale(5, BigDecimal.ROUND_HALF_UP));
			bw.newLine();
			bw.flush();
